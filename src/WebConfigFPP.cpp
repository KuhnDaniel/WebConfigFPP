...
Version 0.2
Author Daniel Kuhn
contakt info@dkNetwork.de

Anpassung für die Steuerung FFP FalconPlayer / RPI / MQTT
mit ArduiTouch von Ziahtec www.zihatec.com / info@zihatec.com 
...
The submitted data will bestored in the SD (chanche from SPIFFS to SD)
The library works with ESP8266 and ESP32

Daniel Kuhn
	char name[20];					// von 15 auf 20 ge
	char label[50];					// von 40 auf 50
	char _buf[2048];				// von 1000 auf 2048
	
Dependencies:
  WebServer.h
  ArduinoJson.h						// Version 6
...
"<title>Falcon Player FPP-RD-Control</title>\n"
"<style>\n"
"body {\n"
...
WebConfigFPP::WebConfigFPP() {
  _apName = "";
};

void WebConfigFPP::setDescription(String parameter){
  ...
  if (error ) {
    ...
    for (JsonObject obj : array) {
      if (i<MAXVALUES) {
        ...
        strlcpy(_description[i].label,obj["label"],50);
...
//both classes have the same functions
  ...
  void WebConfigFPP::handleFormRequest(WebServer * server){
  ...
  void WebConfigFPP::handleFormRequest(WebServer * server, const char * filename){
#else
  //function to respond a HTTP request for the form use the default file
  //to save and restart ESP after saving the new config
  void WebConfigFPP::handleFormRequest(ESP8266WebServer * server){
    handleFormRequest(server,CONFFILE);
  }
  //function to respond a HTTP request for the form use the filename
  //to save. If auto is true restart ESP after saving the new config
  void WebConfigFPP::handleFormRequest(ESP8266WebServer * server, const char * filename){
#endif
...
int16_t WebConfigFPP::getIndex(const char * name){
  int16_t i = _count-1;
...
boolean WebConfigFPP::readConfig(const char * filename){
...
boolean WebConfigFPP::readConfig(){
  return readConfig(CONFFILE);
}
//write configuration to file
boolean WebConfigFPP::writeConfig(const char * filename){
  File f = SD.open(CONFFILE,"w");
  if (f) {
    f.printf("apName=%s\n",_apName.c_str());
    for (uint8_t i = 0; i<_count; i++){
...
boolean WebConfigFPP::writeConfig(){
  return writeConfig(CONFFILE);
}
//delete configuration file
boolean WebConfigFPP::deleteConfig(const char * filename){
  return remove(filename);
}
//delete default configutation file
boolean WebConfigFPP::deleteConfig(){
  return deleteConfig(CONFFILE);
}

//get a parameter value by its name
const String WebConfigFPP::getString(const char * name) {
  int16_t index;
  index = getIndex(name);
  if (index < 0) {
    return "";
...
const char * WebConfigFPP::getValue(const char * name){
  int16_t index;
  index = getIndex(name);
  if (index < 0) {
    return "";
...
const int WebConfigFPP::getInt(const char * name){
  return getString(name).toInt();
}

const float WebConfigFPP::getFloat(const char * name){
  return getString(name).toFloat();
}

const boolean WebConfigFPP::getBool(const char * name){
  return (getString(name) != "0");
}

//get the accesspoint name
const char * WebConfigFPP::getApName(){
  return _apName.c_str();
}
//get the number of parameters
uint8_t WebConfigFPP::getCount(){
  return _count;
}

//get the name of a parameter
String WebConfigFPP::getName(uint8_t index){
  if (index < _count) {
